...
// fake random serial
int fake_random_serial[10] = {1542, 8901, 45097, 6524, 13672, 6421, 26788, 7834, 143, 7752};
...
void init_var_POK_CONFIG_SCHEDULING_NBSLOTS()
{
   ...
   // var_POK_CONFIG_SCHEDULING_NBSLOTS = rand_num%5+3;   // 3~8

   var_POK_CONFIG_SCHEDULING_NBSLOTS = 5;

   printf("var_POK_CONFIG_SCHEDULING_NBSLOTS initialized! Value: %d\n", var_POK_CONFIG_SCHEDULING_NBSLOTS);
}
...
void init_big_random_array()
{
   ...
   printf("big_random_array initialized!\n");
}


// uint64_t pok_sched_slots[4] = (uint64_t[]){2000000000, 40000000000, 1000000000, 10000000000};
uint64_t pok_sched_slots[5];
int rand_index;
void init_pok_sched_slots()
{
   // for(int i=0;i<100;i++) {pok_sched_slots[i] = 99;}   // 99代表无意义的值 跳过
   for(int i=0;i<var_POK_CONFIG_SCHEDULING_NBSLOTS;i++)
   {
      generate_random_number();
      rand_index = rand_num%100;
      pok_sched_slots[i] = (uint64_t)(((big_random_array[rand_index])%4+1)*1000000000);  // 1~4s
   }
   printf("pok_sched_slots initialized! Value: \n");
   printf("[%u", pok_sched_slots[0]);
   for(int i=1;i<var_POK_CONFIG_SCHEDULING_NBSLOTS;i++)
   {
...
// uint8_t pok_sched_slots_allocation[4]  = (uint8_t[]){0, 1, 0, 1};
uint8_t pok_sched_slots_allocation[5];
void init_pok_sched_slots_allocation()
{
   // for(int i=0;i<100;i++) {pok_sched_slots_allocation[i] = 99;}   // 99代表无意义的值 跳过
   for(int i=0;i<var_POK_CONFIG_SCHEDULING_NBSLOTS;i++)
   {
      generate_random_number();
      rand_index = rand_num%100;
      pok_sched_slots_allocation[i] = (uint8_t)((big_random_array[rand_index])%2);
   }

   printf("pok_sched_slots_allocation initialized!\n");
   printf("[%u", pok_sched_slots_allocation[0]);
   for(int i=1;i<var_POK_CONFIG_SCHEDULING_NBSLOTS;i++)
   {
...
void pok_sched_init (void)
{
   ...
   for (slot = 0;slot < var_POK_CONFIG_SCHEDULING_NBSLOTS;slot++)
   {
...
# if POK_CONFIG_NB_PARTITIONS > 1
  ...
  // printf("[DEBUG] %u: ENTER pok_elect_partition()\n", now);

  if (pok_sched_next_deadline <= now)
  {
     // printf("[DEBUG] here, we change the partition\n");
...
#  endif /* defined (POK_NEEDS_PORTS....) */
    pok_sched_current_slot = (pok_sched_current_slot + 1) % var_POK_CONFIG_SCHEDULING_NBSLOTS;
    pok_sched_next_deadline = pok_sched_next_deadline + pok_sched_slots[pok_sched_current_slot];

    // 2020.12.16 dyna debug
   //  printf("[DEBUG] pok_sched_slots=: ");
   // printf("[%u", pok_sched_slots[0]);
   // for(int i=1;i<var_POK_CONFIG_SCHEDULING_NBSLOTS;i++)
   // {
   //    printf(", %u", pok_sched_slots[i]);
   ...
   // printf("[DEBUG] pok_sched_slots_allocation: ");
   // printf("[%u", pok_sched_slots_allocation[0]);
   // for(int i=1;i<var_POK_CONFIG_SCHEDULING_NBSLOTS;i++)
   // {
   //    printf(", %u", pok_sched_slots_allocation[i]);
   ...
   //  printf("[DEBUG] %u: pok_sched_current_slot: %u\n", now, pok_sched_current_slot);
   //  printf("[DEBUG] %u: pok_sched_next_deadline: %u\n", now, pok_sched_next_deadline);
...
void pok_sched()
{
...
#endif /* POK_NEEDS_SCHED_HFPPS */
  {
   ...
   //  printf("[DEBUG] elected_partition: %d\n", elected_partition);
   //  printf("[DEBUG] elected_thread: %d\n", elected_thread);
  }
